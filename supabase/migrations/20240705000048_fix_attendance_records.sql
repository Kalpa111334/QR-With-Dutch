-- First, consolidate existing duplicate records into single records
DO $$
DECLARE
    v_duplicate RECORD;
    ...
    LOOP
        ...
        consolidated AS (
            SELECT 
                ...
                employee_id,
                date,
                MIN(CASE WHEN rn = 1 THEN first_check_in_time END) as first_check_in_time,
                MIN(CASE WHEN rn = 1 THEN first_check_out_time END) as first_check_out_time,
                MIN(CASE WHEN rn = 2 THEN first_check_in_time END) as second_check_in_time,
                MIN(CASE WHEN rn = 2 THEN first_check_out_time END) as second_check_out_time,
                CASE 
                    WHEN MIN(CASE WHEN rn = 2 THEN first_check_out_time END) IS NOT NULL THEN 'CHECKED_OUT'
                    WHEN MIN(CASE WHEN rn = 2 THEN first_check_in_time END) IS NOT NULL THEN 'CHECKED_IN'
                    WHEN MIN(CASE WHEN rn = 1 THEN first_check_out_time END) IS NOT NULL THEN 'ON_BREAK'
                    ELSE 'CHECKED_IN'
                END as status
...
-- Update the function to always maintain a single record per employee per day
