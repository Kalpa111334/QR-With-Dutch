...
BEGIN
    ...
    LOOP
        ...
        consolidated AS (
            SELECT 
                FIRST_VALUE(id) OVER (ORDER BY created_at) as id,
                employee_id,
                date,
                ...
                CASE 
                    WHEN bool_or(rn = 2 AND first_check_out_time IS NOT NULL) THEN 'CHECKED_OUT'
                    WHEN bool_or(rn = 2 AND first_check_in_time IS NOT NULL) THEN 'CHECKED_IN'
                    WHEN bool_or(rn = 1 AND first_check_out_time IS NOT NULL) THEN 'ON_BREAK'
                    ELSE 'CHECKED_IN'
                END as status
