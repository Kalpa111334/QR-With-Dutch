...
BEGIN
    FOR v_duplicate IN 
        ...
        JOIN attendance a2 ON 
            ...
            a1.id != a2.id
    LOOP
        -- Get the ID of the earliest record (this will be our main record)
        ...
        -- Update the main record with consolidated data
        ...
        SET 
            first_check_in_time = (
                SELECT first_check_in_time 
                FROM ordered_records 
                WHERE rn = 1
            ),
            first_check_out_time = (
                SELECT first_check_out_time 
                FROM ordered_records 
                WHERE rn = 1
            ),
            ...
            status = CASE 
                WHEN EXISTS (SELECT 1 FROM ordered_records WHERE rn = 2 AND first_check_out_time IS NOT NULL) THEN 'CHECKED_OUT'
                WHEN EXISTS (SELECT 1 FROM ordered_records WHERE rn = 2 AND first_check_in_time IS NOT NULL) THEN 'CHECKED_IN'
                WHEN EXISTS (SELECT 1 FROM ordered_records WHERE rn = 1 AND first_check_out_time IS NOT NULL) THEN 'ON_BREAK'
                ELSE 'CHECKED_IN'
            END
        WHERE a.id = v_main_record_id;

        -- Delete other records for this employee and date
        DELETE FROM attendance
        WHERE employee_id = v_duplicate.employee_id
        AND date = v_duplicate.date
        AND id != v_main_record_id;
    END LOOP;
END $$;
